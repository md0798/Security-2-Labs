...
def main():
    ...
    plt.imshow(exam_data[0][0],cmap='gray',interpolation='none')
    plt.savefig("Original.png")
    exam_data_fgm = fast_gradient_method(net,exam_data,0.2,np.inf)
    exam_data_fgm = (np.clip(exam_data_fgm.detach().numpy(),0,1))*255
    plt.imshow(exam_data_fgm[0][0],cmap='gray',interpolation='none')
    plt.savefig("fgsm_0.2.png")
    exam_data_fgm = fast_gradient_method(net,exam_data,2,2)
    exam_data_fgm = (np.clip(exam_data_fgm.detach().numpy(),0,1))*255
    plt.imshow(exam_data_fgm[0][0],cmap='gray',interpolation='none')
    plt.savefig("fgsm_2.png")
    exam_data_pgd = projected_gradient_descent(net,exam_data,0.2,0.01, 40,np.inf)
    exam_data_pgd = (np.clip(exam_data_pgd.detach().numpy(),0,1))*255
    plt.imshow(exam_data_pgd[0][0],cmap='gray',interpolation='none')
    plt.savefig("pgd_0.2.png")    
    exam_data_pgd = projected_gradient_descent(net,exam_data,2,0.01, 40,2)
    exam_data_pgd = (np.clip(exam_data_pgd.detach().numpy(),0,1))*255
    plt.imshow(exam_data_pgd[0][0],cmap='gray',interpolation='none')
...
'''
    for x, y in data.test:
        ...
        x_fgm = fast_gradient_method(net, x, 0.2, np.inf)
        x_pgd = projected_gradient_descent(net, x, 0.2, 0.01, 40, np.inf)
        x_fgm_2 = fast_gradient_method(net, x, 2, 2)
        x_pgd_2 = projected_gradient_descent(net, x, 2, 0.01, 40, 2)
